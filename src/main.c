...
// STATIC ALLOCATIONS: Placed in BSS memory to prevent eZ80 stack overflow
static uint8_t comp_buf[16384];
...
void display_error(const char *reason) {
    ...
    // Save Log AppVar in RAM before halting
...
bool open_chunk_ptr(ChunkReader *r, uint8_t chunk_idx) {
    ...
    // Open file from Archive or RAM
    r->file = ti_Open(var_name, "r");
    if (!r->file) {
        return false;
    }
    
    // Direct pointer to AppVar payload in Flash or RAM
    r->data = (uint8_t *)ti_GetDataPtr(r->file);
    r->size = ti_GetSize(r->file);
    r->pos = 0;
    r->current_chunk = chunk_idx;
    log_msg("Opened chunk: %s (Size: %u)", var_name, (unsigned int)r->size);
    return true;
}
...
void setup_grayscale_palette(void) {
    for (int i = 0; i < 256; i++) {
        ...
        uint8_t g = i >> 2;
        uint8_t b = i >> 3;
        uint16_t color1555 = (1 << 15) | (r << 10) | (g << 5) | b;
        gfx_palette[i] = color1555;
    }
}
...
void play_video(const char *prefix) {
    ...
    log_msg("Starting playback for prefix: %s", reader.prefix);
    ...
    if (!read_bytes_safe(&reader, magic, 6) ||
        ...
        display_error("Failed to read header");
        return;
    }
    
    if (memcmp(magic, "CEVID1", 6) != 0) {
        display_error("Magic header mismatch (Not a valid CEVID file)");
        return;
    }
    
    if (width != 320 || height != 240) {
        display_error("Invalid resolution (Must be 320x240)");
        return;
    }
    ...
    for (uint32_t frame = 0; frame < total_frames; frame++) {
        ...
        if (!read_bytes_safe(&reader, &comp_len, 2)) {
            gfx_End();
            display_error("Unexpected EOF reading frame length");
            return;
        }
        
        if (comp_len > sizeof(comp_buf)) {
            gfx_End();
            display_error("Frame size exceeds max buffer limit");
            return;
        }
        
        if (!read_bytes_safe(&reader, comp_buf, comp_len)) {
            gfx_End();
            display_error("Unexpected EOF reading frame payload");
            return;
        }
        
        decompress_rle_delta(comp_buf, comp_len, frame_buf, sizeof(frame_buf));
        
        // Copy decompressed frame to VRAM draw buffer and blit
        memcpy(gfx_vbuffer, frame_buf, sizeof(frame_buf));
        gfx_SwapDraw();
    }
    
    gfx_End();
    if (reader.file) ti_Close(reader.file);
    log_msg("Playback finished normally");
}
